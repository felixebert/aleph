...
function computePercentCompleted(uploadTraces, totalUploadSize) {
  ...
  let reduce = uploadTraces.reduce((result, trace) => {
    let summand = 0;
    ...
    console.log(`${trace.name}: ${summand}, status: ${trace.status}, uploaded: ${trace.uploaded}, total: ${trace.total}, size: ${trace.size}`);
